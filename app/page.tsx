...
export default function TelegramWarehouseApp() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      <SearchHeader searchQuery={searchQuery} onSearchChange={setSearchQuery} user={user} />

      <main className="pt-16">
        {currentView === "catalog" && <ProductCatalog searchQuery={searchQuery} />}
        {currentView === "cart" && <Cart />}
        {currentView === "orders" && <OrderFlow />}
      </main>

      <BottomNav currentView={currentView} onViewChange={setCurrentView} cartItemCount={totalItems} />
    </div>
  )
}
